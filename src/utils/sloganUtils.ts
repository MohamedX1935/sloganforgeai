...
export const downloadSloganAsPDF = (
  ...
  const file = new Blob([
    `Votre Slogan: ${slogan}\n\n` + 
    `Font: ${options.font}\n` +
    `Couleur: ${options.color}\n` +
    `Taille: ${options.size}px\n` +
    `Format: ${options.format}\n` +
    `Alignement: ${options.alignment || 'center'}\n` +
    `Couleur de fond: ${options.backgroundColor || '#FFFFFF'}\n` +
    `Position du logo: ${options.logoPosition || 'top'}\n\n` +
    `Généré par SloganForge AI`
  ], {type: 'text/plain'});
